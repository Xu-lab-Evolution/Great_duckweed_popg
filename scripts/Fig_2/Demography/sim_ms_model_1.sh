...
do
	...
	length="$(cut -f 3 -d "," <<< $lineRecomb)"
        ...
        $msms_exec -N $N_msms -ms 68 1 -t $theta -r $rho $length -I 4 17 17 17 17 -n 2 $N_ASI_prop -n 3 $N_EUR_prop -n 4 $N_IND_prop -ma x $N_migA $N_migE $N_migI $N_migA x $N_migE $N_migI $N_migE $N_migE x 0 $N_migI $N_migI 0 x -SI $timecoal_EUR 4 0.1 0.1 0.1 0.1 -SAA $SAA -SaA $Shet -en $timebn 4 1 -en $timebn 3 1 -en $timebn 2 1 -en $timebn 1 1 -ej $timecoal_EUR 3 2 -ej $timecoal_IND 4 2 -ej $timecoal_ASIA 2 1 > out.ms.tmp.$1
